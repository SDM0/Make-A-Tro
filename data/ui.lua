...
function G.UIDEF.mat_used_materials()
	...
	if G.GAME["used_mat"] then
		for i = 1, math.ceil(#G.GAME["used_mat"] / (5 * #G.your_collection)) do
			table.insert(obj_options, localize('k_page') .. ' ' .. tostring(i) .. '/' .. tostring(math.ceil(#G.GAME["used_mat"] / (5 * #G.your_collection))))
		end

		for i = 1, 5 do
			for j = 1, #G.your_collection do
				if G.GAME["used_mat"][i + (j - 1) * 5] then
					local card = Card(G.your_collection[j].T.x + G.your_collection[j].T.w / 2, G.your_collection[j].T.y, G.CARD_W, G.CARD_H, nil, G.P_CENTERS[G.GAME["used_mat"][i + (j - 1) * 5]])
					silent = true
					G.your_collection[j]:emplace(card)
				end
			end
			...
			nodes = {
				{
					...
					config = { align = "cm" },
					nodes = {
						{ n = G.UIT.O, config = { object = DynaText({ string = { (#G.GAME["used_mat"] > 1 and localize('ph_mats_redeemed_plural') or localize('ph_mats_redeemed_singular')) }, colours = { G.C.UI.TEXT_LIGHT }, bump = true, scale = 0.6 }) } }
					}
				},
				{
					n = G.UIT.R,
