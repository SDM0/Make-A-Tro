...
SMODS["Mat_obj"] = SMODS.Consumable:extend({
    ...
    in_pool = function(self)
        ...
        local used = G.GAME["used_mat_" .. self.mat_type] or {}
        for _, joker in ipairs(G.jokers.cards or {}) do
            if joker.config.center.key == "j_mat_custom_joker" then
                if self.key == joker.ability.extra[self.mat_type].key then
                    return false
                end
            end
        end
        for _, k in ipairs(used) do
            if self.key == k then return false end
        end
        return true
    end,
    ...
    set_card_type_badge = function(self, card, badges)
        badges[#badges+1] = create_badge(
            ...
            (self.mat_type == 'hat' and G.C.RED) or (self.mat_type == 'head' and G.C.ORANGE) or (self.mat_type == 'collar' and G.C.BLUE), nil, 1.2
        )
